		...
		(
			SELECT ho.bticode AS "Код БТИ"
				...
				WHERE (
					...
					OR ho.heating_area > ho.full_area
					...
					OR (
						(SELECT count(he4.bticode) FROM heat_raw AS he4 WHERE he4.bticode = ho.bticode) > 
						(SELECT count(ho4.bticode) FROM house_raw AS ho4 WHERE ho4.bticode = ho.bticode)
					)
					...
					AND (
						(
							(ho.category NOT LIKE '%едомств%')
							AND (ho.category NOT LIKE '%бщежитие%')
							AND (ho.category NOT LIKE '%остин%')
							AND (ho.category NOT LIKE '%часный ж/ф%')
							AND (ho.category NOT LIKE '%частный%')
							AND (ho.category NOT LIKE '%договор%')
						)	
...
copy ( SELECT (
			      ...
			      (CASE t."Корпус" IS NULL WHEN TRUE THEN 1 ELSE 0 END ) +
				(CASE (SELECT ho2.stroenie FROM house_raw AS ho2 
				WHERE ho2.bticode = t."Код БТИ" LIMIT 1) IS NULL WHEN TRUE THEN 1 ELSE 0 END ) +
			      (SELECT (CASE ((ho2.bticode IS NOT NULL) AND (he2.bticode IS NULL)) WHEN TRUE THEN 52 ELSE 0 END )
				FROM house_raw AS ho2
				LEFT JOIN heat_raw AS he2 ON ho2.bticode = he2.bticode
				WHERE ho2.bticode = t."Код БТИ" LIMIT 1) +
		...
		FROM
			...
			GROUP BY "Район", "Улица", "Дом", "Корпус", "Код БТИ"
		    ) AS t
		)
		+
		(
